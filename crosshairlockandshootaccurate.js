...
const demoBoneHeads = [
  ...
  // có thể thêm nhiều enemy khác ở đây
];
...
function mainLoop() {
  if (!isCrosshairRed) {
    // Nếu tâm ngắm không đỏ, reset lock
    lockedTarget = null;
    targetHistory = [];
  ...
  // Chọn target head gần nhất trong phạm vi lock
  let bestTarget = chooseBestHeadTarget(crosshair);

  if (!bestTarget) {
    // Không tìm thấy target hợp lệ => reset lock
    lockedTarget = null;
    targetHistory = [];
    setTimeout(mainLoop, 16);
    return;
  ...
  // Cập nhật tâm ngắm dịch theo aimDelta
  const newCrosshair = crosshair.add(aimDelta);
  crosshairLock.crosshair = newCrosshair;

  // Kiểm tra nếu vượt phạm vi reset thì reset lock
